...
class PointCloudObjectRetrieval():
    ...
    # def get_aspect_ratio(pcd_obj):
    #     pcd_sampling = np.random.choice(len(pcd_obj), 
    #         size=min(int(len(pcd_obj)*0.5), 500), replace=False) #min([int(len(point_cloud)*0.2), 1000]), replace=False)
    #     eigvals, eigvecs = np.linalg.eig(np.cov(pcd_obj[pcd_sampling].T))
    #     orientation = atan2(eigvecs[0,1], eigvecs[0,0])
    #     return (
    #         [eigvals[0], eigvals[1], eigvals[2], 
    #         (eigvals[0]-eigvals[1])/eigvals[0], eigvals[2]/eigvals[0], eigvals[2]/eigvals[1], orientation]
    #     )

    # def get_outline_ratio(pcd_obj):
    #     #convert 2d projection to image
    #     #shift X and Y coordinate to 0 origin
    #     X_shift = pcd_obj[:,0]-min(pcd_obj[:,0]); Y_shift = pcd_obj[:,1]-min(pcd_obj[:,1])
    ...
    # def get_zeboudj_distance(pcd_obj, distance = 2.5):
    #     #build kdtree for each cluster
    #     pcd_sampling = np.random.choice(len(pcd_obj), size=min(int(len(pcd_obj)*0.5), 10000), replace=False)
    #     cl_sample = pcd_obj[pcd_sampling]
    ...
    # def get_depth_distribution(pcd_obj):
    #     zmin = np.percentile(pcd_obj[:,2], 2)
    #     zmax = np.percentile(pcd_obj[:,2], 98)
    # #     print("zmin {} zmax {}".format(zmin, zmax))
    #     r_zlow = len(pcd_obj[pcd_obj[:,2]<zmin+(zmax-zmin)/4])/len(pcd_obj) #proportion_under_one_fourth
    #     r_zhigh = len(pcd_obj[pcd_obj[:,2]>zmax-(zmax-zmin)/4])/len(pcd_obj) #proportion_over_three_fourth
    # #     print("r_zlow {} r_zhigh {}".format(r_zlow, r_zhigh))
    #     return([r_zlow, r_zhigh])

    # def extract_features(self):
    #     n_points = len(self.pcd)
    #     amp_mean = self.pcd[:,3].mean()
    #     semb_mean = self.semblance.mean()
    #     zeboudj_2_5 = get_zeboudj_distance(self.pcd, distance=2.5)
    ...
    def get_features(self, selected_clusters):
        ...
        featureDF = pd.DataFrame(data=Features, columns=[
            "segmentID", "nPoints", "amplitudeMean", "semblanceMean", "ZeboudjDistance", 
            "outlineRatio", "lambda1", "lambda2", "lambda3", "linearity",
            "slope", "slopeMinAxis", "orientation", "rZHigh", "rZLow",
        ])
